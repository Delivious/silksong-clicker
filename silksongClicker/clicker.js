...
function getGameData() {
  return {
    ...
    sharpenMultiplier: sharpenMultiplier,
    distance: distance,
    threefold: threefold,
    threefoldCost: threefoldCost,
    threefoldCount: threefoldCount,
    sharpenCost: sharpenCost,
    sharpenCount: sharpenCount,
    sharp: sharp,
    thrower: thrower,
    throwerCost: throwerCost,
    throwerCount: throwerCount,
    clickcost: clickcost,
    noiseMode: noiseMode,
    rps: rps,
    rpsCounter: rpsCounter,
    rpsPercent: rpsPercent,
    rpsRebirth: rpsRebirth,
    ...
    bought3: bought3,
    bought4: bought4,
    paleOilCost: paleOilCost,
    paleOilCount: paleOilCount,
    purchase1: purchase1,
    purchase2: purchase2,
    purchase3: purchase3,
    purchase4: purchase4,
...
async function loadGame() {
  ...
  roseValue = s.roseValue ?? roseValue
  multiplier = s.multiplier ?? multiplier
  sharpenMultiplier = s.sharpenMultiplier ?? sharpenMultiplier
  distance = s.distance ?? distance

  threefoldCost = s.threefoldCost ?? threefoldCost
  threefoldCount = s.threefoldCount ?? threefoldCount

  sharpCost = s.sharpenCost ?? sharpCost
  sharpCount = s.sharpenCount ?? sharpCount

  throwerCost = s.throwerCost ?? throwerCost
  throwerCount = s.throwerCount ?? throwerCount

  clickcost = s.clickcost ?? clickcost

  rps = s.rps ?? rps
  rpsCounter = s.rpsCounter ?? rpsCounter

  rebirthCount = s.rebirthCount ?? rebirthCount
  rebirthCost = s.rebirthCost ?? rebirthCost
  rebirthTokens = s.rebirthTokens ?? rebirthTokens

  paleOilCost = s.paleOilCost ?? paleOilCost
  paleOilCount = s.paleOilCount ?? paleOilCount

  purchase1 = s.purchase1 ?? purchase1
  purchase2 = s.purchase2 ?? purchase2
  purchase3 = s.purchase3 ?? purchase3
  purchase4 = s.purchase4 ?? purchase4
  purchase5 = s.purchase5 ?? purchase5

  bought1 = s.bought1 ?? bought1
  bought2 = s.bought2 ?? bought2
  bought3 = s.bought3 ?? bought3
  bought4 = s.bought4 ?? bought4

  console.log("Game loaded")
  // restart intervals if needed
  if (throwerCount > 0 && !thrower) {
    thrower = setInterval(() => {
